    ...
    <footer className="pt-24 pb-12 px-6 border-t border-slate-100 bg-[var(--bg-main)]">
      <div className="max-w-7xl mx-auto px-6 lg:px-12">
        ...
        <div className="pt-10 border-t border-slate-200 flex flex-col md:flex-row justify-between items-center gap-6">
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em]">© 2026 HiveHr Agriculture Infrastructure Inc.</p>
          <div className="flex gap-8">
              <a href="#" className="text-[10px] font-bold text-slate-400 hover:text-[var(--primary)] transition-colors uppercase tracking-widest">Privacy Policy</a>
              <a href="#" className="text-[10px] font-bold text-slate-400 hover:text-[var(--primary)] transition-colors uppercase tracking-widest">Cookie Policy</a>
          </div>
        </div>
      </div>
    </footer>
...
const FooterCol = ({ title, links }: { title: string, links: { name: string, href: string }[] }) => (
  <div className="text-left">
      <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-900 mb-8">{title}</h4>
      <ul className="space-y-4">
          {links.map((link) => (
              <li key={link.name}>
                  <Link to={link.href} className="text-[13px] font-bold text-slate-400 hover:text-[var(--primary)] transition-colors tracking-tight">{link.name}</Link>
              </li>
          ))}
      </ul>
  </div>
